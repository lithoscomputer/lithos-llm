...
//!
//! Every roster model claims structured output. The schema is the wire
//! suite's `city_report`, so live and mocked output pin the same shape.
...
async fn produces_parseable_json(model: &str) -> TestResult {
    if !anthropic::capabilities(model).structured_output {
