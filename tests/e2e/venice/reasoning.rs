...
//!
//! 1. Presence: fabro's catalog says the eight established rows reason by
//!    default, so those assert reasoning evidence; the eight new rows only
//!    record what they observe, until a first nightly run pins them.
...
///
/// fabro's catalog and Venice's live listing disagree about the kimi rows
...
async fn replays_reasoning_with_a_tool_result(model: &str) -> TestResult {
    ...
    let second = venice::request(model)
        .user("Look up the weather in Paris, then tell me if it suits a picnic.")
        .tool(tools_weather())
