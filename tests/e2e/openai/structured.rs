...
//!
//! Runs on every roster model; each row claims `structured_output`, verified
//! by raw `json_schema` probes on 2026-08-30 (the pro rows included). The
...
fn assert_city_report(payload: &Value) {
    ...
    // Conformance to the schema is the assertion; plausibility is not.
    assert!(
