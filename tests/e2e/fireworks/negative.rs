...
//! tests verify the shapes Fireworks sends today still land in the right
//! [`ErrorKind`].
...
async fn a_bad_key_classifies_as_authentication() -> TestResult {
    ...
    let client = fireworks::client_with_key("lithos-e2e-invalid-key");
    let request = fireworks::request(MODEL).user("Hello").build()?;
    let error = client
        .complete(request)
        .await
    ...
    assert_eq!(
        ...
        "Fireworks's live 401 shape classified as {:?}: {}",
        error.kind(),
        error.message()
    );
    Ok(())
...
async fn an_unknown_passthrough_model_classifies_cleanly() -> TestResult {
    ...
    // The provider allows passthrough, so this reaches the wire and Fireworks
    // itself refuses the unknown id.
    let request = lithos_llm::Request::builder()
        .model(fireworks::selector("lithos-e2e-does-not-exist"))
        .user("Hello")
    ...
    assert!(
        ...
        "Fireworks's unknown-model shape classified as {:?}: {}",
        error.kind(),
        error.message()
    );
    Ok(())
...
/// Records whether Fireworks populates rate-limit headers. Nothing pins this
...
async fn probe_rate_limit_headers() -> TestResult {
    ...
    match &response.rate_limits {
        Some(limits) => support::observe(&format!("fireworks rate limits: {limits:?}")),
        None => support::observe("fireworks sent no rate-limit headers"),
    }
    Ok(())
}
