//! V2 — tool calling.
//!
//! Fireworks accepts automatic, required, and named tool selection, parallel
//! calls, and tool result turns through its OpenAI-compatible endpoint.
...
use crate::fireworks::{self, family_tests, model_tests};
...
async fn issues_parallel_tool_calls(model: &str) -> TestResult {
    ...
    if model == "gpt-oss-120b" {
        return support::skip(
            "Fireworks GPT-OSS 120B issued one call per turn in two live probes on 2026-08-29",
        );
    }
