...
async fn sampling_is_rejected_locally_where_unclaimed() -> TestResult {
    let client = fireworks::client_with_key("preflight-key");
...
/// The built-in Fireworks row works end to end: `Client::from_env` resolves
/// the conventional `FIREWORKS_API_KEY` mapping, the default route resolves,
/// and catalog pricing produces a cost.
#[tokio::test]
#[ignore = "live Fireworks call; run with `mise run test:e2e`"]
async fn the_builtin_catalog_reaches_fireworks() -> TestResult {
    if let Some(skip) = support::live_only("the unproxied built-in base URL") {
        return skip;
    }
    if env::var(fireworks::KEY_VARIABLE).is_err() {
...
/// Every configured wire id must be available from Fireworks before the E2E
...
async fn the_live_listing_contains_every_configured_wire_id() -> TestResult {
    ...
    let Ok(key) = env::var(fireworks::KEY_VARIABLE) else {
        return support::skip("FIREWORKS_API_KEY is unset");
    };
    ...
    let live: Vec<&str> = listing["data"]
        ...
        .collect();
    let catalog = fireworks::catalog();
    let provider = catalog.provider(fireworks::PROVIDER)?;
