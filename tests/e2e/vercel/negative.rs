...
//! tests verify the shapes the gateway sends today still land in the right
//! [`ErrorKind`].
...
async fn a_bad_key_classifies_as_authentication() -> TestResult {
    ...
    let client = vercel::client_with_key("lithos-e2e-invalid-key");
    let request = vercel::request(MODEL).user("Hello").build()?;
    let error = client
        .complete(request)
        .await
    ...
    assert_eq!(
        ...
        "the gateway's live 401 shape classified as {:?}: {}",
        error.kind(),
        error.message()
    );
    Ok(())
...
async fn an_unknown_passthrough_model_classifies_cleanly() -> TestResult {
    ...
    // The provider allows passthrough, so this reaches the wire and the
    // gateway itself refuses the unknown id with a 404 `model_not_found`.
    let request = lithos_llm::Request::builder()
        .model(vercel::selector("lithos-e2e-does-not-exist"))
        .user("Hello")
    ...
    assert!(
        ...
        "the gateway's unknown-model shape classified as {:?}: {}",
        error.kind(),
        error.message()
    );
    Ok(())
...
/// Records whether the gateway populates rate-limit headers. Nothing pins this
...
async fn probe_rate_limit_headers() -> TestResult {
    ...
    match &response.rate_limits {
        Some(limits) => support::observe(&format!("vercel rate limits: {limits:?}")),
        None => support::observe("vercel sent no rate-limit headers"),
    }
    Ok(())
}
