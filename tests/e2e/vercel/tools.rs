...
/// The skip for the cells that force a call. The Fable 5.1 upstream answers
/// a forced choice with a 400, which the gateway passes through, so its row
/// says so and these cells skip it.
fn forced_choice_unclaimed(model: &str) -> Option<TestResult> {
    let capabilities = vercel::capabilities(model);
...
async fn calls_the_forced_tool(model: &str) -> TestResult {
    ...
    // The gateway can select another upstream between attempts. One miss gets
...
async fn uses_the_tool_result(model: &str) -> TestResult {
    if !vercel::capabilities(model).tools().is_supported() {
...
async fn issues_parallel_tool_calls(model: &str) -> TestResult {
    ...
    // Nemotron issued one call per turn on four of five attempts and Laguna S
    // on two of four through the gateway on 2026-09-17: neither batches the
    // two lookups reliably, so do not require model-level batching.
    if matches!(model, "nemotron-3-super-120b-a12b" | "laguna-s-2.1") {
        return support::skip("the Vercel route does not reliably issue parallel tool calls");
    }
    let Some(client) = vercel::live_client() else {
        return support::skip("AI_GATEWAY_API_KEY is unset");
    };
