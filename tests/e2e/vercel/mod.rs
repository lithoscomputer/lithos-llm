...
//! cache counters it passes through, and the classification of its error
//! payloads.
//!
//! The roster lives in `vercel_catalog.toml`; the suite iterates it through
...
pub(crate) const PROVIDER: &str = "vercel";

/// The environment variable holding the Vercel AI Gateway API key.
pub(crate) const KEY_VARIABLE: &str = "AI_GATEWAY_API_KEY";

const CATALOG_TOML: &str = include_str!("../vercel_catalog.toml");
...
macro_rules! family_tests {
    ($runner:path) => {
        crate::vercel::model_tests!(@expand $runner,
            claude_sonnet_5 "claude-sonnet-5",
            gpt_5_6_luna "gpt-5.6-luna",
            gemini_3_5_flash "gemini-3.5-flash",
            mimo_v2_5_pro "mimo-v2.5-pro",
...
fn twin_url() -> String {
    env::var("LITHOS_VERCEL_E2E_TWIN_URL").unwrap_or_else(|_| "http://127.0.0.1:3927".to_owned())
}

/// The Vercel E2E catalog.
///
...
pub(crate) fn catalog() -> Catalog {
    let base = Catalog::builder()
        .toml_layer("vercel-e2e", CATALOG_TOML)
        .expect("the Vercel E2E catalog should parse");
    let builder = if support::Backend::from_env() == support::Backend::Live {
        base
    } else {
        base.toml_layer(
            "e2e-twin",
            &format!("[providers.vercel]\nbase_url = \"{}\"\n", twin_url()),
        )
        .expect("the twin overlay should parse")
    };
    builder
        .build()
        .expect("the Vercel E2E catalog should validate")
}
...
pub(crate) fn capabilities(model: &str) -> ModelCapabilities {
    catalog()
        ...
        .expect("the roster model should be in the Vercel E2E catalog")
        .capabilities()
}

/// A client for this run's backend, or `None` when a live run has no key.
///
/// Live runs authenticate with `AI_GATEWAY_API_KEY`. Record and replay runs go
...
///
/// The negative suite uses this directly to observe how the gateway answers a
/// bad credential.
pub(crate) fn client_with_key(key: &str) -> Client {
    let build = Client::builder()
        ...
        .build()
        .expect("the Vercel E2E client should build");
    assert!(
        build.issues.is_empty(),
        "the Vercel E2E client reported provider build issues: {:?}",
        build.issues
    );
    build.client
}
