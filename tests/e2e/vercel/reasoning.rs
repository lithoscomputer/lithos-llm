...
mod effort_levels {
    ...
    level_tests!(
        ...
        qwen_3_8_max_low "qwen3.8-max" Low,
...
/// Effort probes for the still-unpinned rows.
///
/// The gateway maps `reasoning_effort` onto each upstream's control and
/// accepted levels the listing omits (Kimi K3 and GLM 5.3 both took
/// `medium` on 2026-09-17), so the listing's vocabulary is a floor, not a
/// contract. A probe sends one low-effort and one high-effort request and
/// records what came back; it fails only on an error that is neither
/// acceptance nor a clean invalid request, so its output settles the catalog
/// without turning a vocabulary question into a red nightly.
mod effort_probes {
    use super::*;
...
/// The roster rows the catalog marks `reasoning_by_default`, where missing
/// reasoning evidence is a failure rather than an observation.
...
async fn shows_reasoning_evidence(model: &str) -> TestResult {
    ...
    let request = vercel::request(model).user(HARD_PROMPT).build()?;
...
async fn accepts_the_effort_level(model: &str, effort: ReasoningEffort) -> TestResult {
    ...
    // Reasoning evidence is either bucket: the gateway counts reasoning
    // tokens in usage for every upstream, but the Claude rows return no
    // reasoning text at all, so the count is the only evidence there.
...
async fn replays_reasoning_with_a_tool_result(model: &str) -> TestResult {
    let opening = vercel::request(model)
...
async fn replays_reasoning_from(model: &str, opening: RequestBuilder) -> TestResult {
    ...
    let second = vercel::request(model)
        .user("Look up the weather in Paris, then tell me if it suits a picnic.")
        .tool(tools_weather())
