...
//!
//! Runs on every roster model whose catalog row claims the format; a row
//! without the claim (Laguna, Kimi K2.6) is skipped here and rejected
//! client-side by the preflight suite. Rows that claim only `json_object`
//! (MiMo, GLM 4.6, Qwen3 Coder, Qwen 3.8 27B) run the first cell only. The
//! schema is the wire suite's `city_report`, so live and mocked structured
//! output pin the same shape.
...
async fn produces_parseable_json(model: &str) -> TestResult {
    if !vercel::capabilities(model)
        .response_format(&ResponseFormat::JsonObject)
        .is_supported()
    {
        return support::skip("the catalog does not claim json_object output");
    }
