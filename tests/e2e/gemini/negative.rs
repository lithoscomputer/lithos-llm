...
//!
//! These tests verify that Gemini's live error envelopes still land in the
//! right [`ErrorKind`].
...
async fn a_bad_key_classifies_as_authentication() -> TestResult {
    ...
    let client = gemini::client_with_key("lithos-e2e-invalid-key");
    let request = gemini::request(MODEL).user("Hello").build()?;
    let error = client
        .complete(request)
        .await
    ...
    assert_eq!(
        ...
        "Gemini's live 401 shape classified as {:?}: {}",
        error.kind(),
        error.message()
    );
    Ok(())
...
async fn an_unknown_passthrough_model_classifies_cleanly() -> TestResult {
    ...
    // The provider allows passthrough, so this reaches the wire and Gemini
    // itself refuses the unknown id.
    let request = lithos_llm::Request::builder()
        .model(gemini::selector("lithos-e2e-does-not-exist"))
        .user("Hello")
    ...
    assert!(
        ...
        "Gemini's unknown-model shape classified as {:?}: {}",
        error.kind(),
        error.message()
    );
    Ok(())
...
/// Records whether Gemini populates rate-limit headers. Nothing pins this
...
async fn probe_rate_limit_headers() -> TestResult {
    ...
    match &response.rate_limits {
        Some(limits) => support::observe(&format!("gemini rate limits: {limits:?}")),
        None => support::observe("gemini sent no rate-limit headers"),
    }
    Ok(())
}
