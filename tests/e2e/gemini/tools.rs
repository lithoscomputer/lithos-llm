...
use lithos_llm::types::{
    ContentPart, FinishReason, Message, Role, ToolChoice, ToolDefinition, ToolResult,
};
use serde_json::{Value, json};

use crate::gemini::{self, family_tests, model_tests};
...
async fn uses_the_tool_result(model: &str) -> TestResult {
    ...
    // Gemini 3 validates the thought signature on a model-generated tool
    // call. Start with a real forced call instead of injecting a synthetic
    // assistant turn.
    let opening = client
        .complete(
...
async fn acknowledges_an_error_tool_result(model: &str) -> TestResult {
    ...
    let request = gemini::request(model)
        ...
        .build()?;
    let response = client.complete(request).await?;
    // Retrying the lookup is as legitimate as answering in text.
    assert!(
