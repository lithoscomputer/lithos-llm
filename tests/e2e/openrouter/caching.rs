//! V2 — prompt caching.
//!
//! Caching on OpenRouter is automatic where it exists at all, so the test is a
...
//! prefix is the expensive part of the whole OpenRouter suite.

use std::time::Duration;

use tokio::time::sleep;

use crate::openrouter::{self, family_tests};
...
/// on OpenRouter's Claude path on every call and never read one, while varied
...
async fn caches_a_shared_prefix(model: &str) -> TestResult {
    ...
    // OpenRouter derives sticky routing from the opening messages after it
    // observes cache use. This pair exercises that documented default without
    // claiming Lithos's `prompt_cache_key`-based cache-routing capability.
    let first = client
        .complete(
...
async fn auto_cache_off_is_accepted() -> TestResult {
    ...
    let request = openrouter::request("deepseek-v4-flash")
        ...
        .provider_option(openrouter::PROVIDER, "auto_cache", serde_json::json!(false))
        .build()?;
